...
//				}
        ...
        private String getStreamName(String name, String packetizer, String repeater)
        {
            ...
            String url = lookupURL(nameContext, false);
        ...
        private void resolveMediaCasterURL(final LiveMediaStreamReceiver mediaCaster)
        {
            appInstance.getVHost().getThreadPool().execute(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    String newURL = lookupURL(streamName, true);
...
//					writeLock.writeLock().unlock();
    ...
    class Lookup extends Thread
    {
        ...
        public void run()
        {
            ...
            final CompletionService<String> ecs = new ExecutorCompletionService<>(executor);
        ...
        {
            return "Lookup: {streamName: " + streamName + ", name: " + streamName + ", startTime: " + startTime + ", url: " + url + "}";
        }
    }
    ...
    private String lookupURL(String nameContext, boolean forceNewLookup)
    {
        Lookup lookup = null;
        if (debug)
            logger.info(MODULE_NAME + ".lookupUrl Looking up url for [" + appInstance.getContextStr() + "/" + nameContext + "] lookups: " + lookups.toString() + ", urls: " + urls.toString(), WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
        synchronized (lock)
        {
            if (!forceNewLookup && urls.containsKey(nameContext))
            {
                ...
                if (debug)
                    logger.info(MODULE_NAME + ".lookupUrl from urls list, returning url for [" + appInstance.getContextStr() + "/" + nameContext + "] url: " + url, WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
                return url;
            }
            ...
            if (lookup == null)
            {
                lookup = new Lookup(nameContext);
                lookup.start();
                lookups.put(nameContext, lookup);
                if (debug)
                    logger.info(MODULE_NAME + ".lookupUrl adding new lookup url for [" + appInstance.getContextStr() + "/" + nameContext + "] lookup: " + lookup.toString(), WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
            }
        }
        try
        {
            if (debug)
                logger.info(MODULE_NAME + ".lookupUrl joining existing lookup for [" + appInstance.getContextStr() + "/" + nameContext + "] lookup: " + lookup.toString(), WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
            lookup.join(timeout);
        ...
        if (debug)
            logger.info(MODULE_NAME + ".lookupUrl after lookup, returning url for [" + appInstance.getContextStr() + "/" + nameContext + "] url: " + lookup.url, WMSLoggerIDs.CAT_application, WMSLoggerIDs.EVT_comment);
                ...
                if (!renditions.isEmpty())
                {
                    ...
                    for (MediaListRendition rendition : renditions)
                    {
                        if (rendition.getType() == IVHost.CONTENTTYPE_AUDIO || rendition.getType() == IVHost.CONTENTTYPE_VIDEO)
                        {
                            ...
                            if (appInstance.resolvePlayAlias(renditionName, httpSession) != null)
                                availableCount++;
                        }
                    }
                    if (renditionCount > 0)
